...
import com.gradpro.connector.Connector;
import com.gradpro.vo.Notice;
...
public class TaskDAO {
    ...
    private static final String UPDATE_PF_TASK ="update task set title=?,summary=?,status='1',level=? where taskid=?";

    private static final String GET_TEACHERID_BY_STUDENTID = "select teaid from task where stuid=?";
    private static final String GET_STUDENT_NUMBER_BY_TEACHER_ID = "select count(*) from task where teaid=? and status<>10";
        ...
        try {
            ...
            if (rs.next()) {
                task = new Task(rs.getString("taskid"), rs.getString("title"), rs.getString("summary"), 
                                ...
                                rs.getDate("subtime").toString() + " " + rs.getTime("subtime").toString(),rs.getString("level"));
    ...
    public void updateTaskPF(String taskid, String tasktitle, String summary, String level) throws SQLException {
        Connection conn = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            ps = conn.prepareStatement(UPDATE_PF_TASK);
            ps.setString(1, tasktitle);
            ps.setString(2, summary);
